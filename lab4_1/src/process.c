...
//------------------------------------------
	...
	for (i = 0; i < L1_MAX_ENTRIES; i++) {		// for loop until max number of entries has been reached
		if(pcb->pagetable[i] != 0) {			// if entry in the table has something in it
			MemoryFreePte (pcb->pagetable[i]);	// free the memory
		}
	}
	pcb->npages = 0;							// reset number of pages allocated to 0
...
//------------------------------------------------------------
	...
	pcb->npages = 4;						// set number of pages to 4
  
	for (i = 0; i < L1_MAX_ENTRIES; i++)	// for loop to ensure all pages in table are empty
		pcb->pagetable[i] = 0x0;			// access pagetable and set to zero

	for(i = 0; i < 3; i++) {				// for loop allocating 3 pages for text and data
		newPage = MemoryAllocPage ();		// allocate new page
		if (newPage == 0) {					// if page was unable to be allocated
		printf ("aFATAL: couldn't allocate memory - no free pages!\n");
		exitsim ();	// NEVER RETURNS!
		}
    pcb->pagetable[i] = MemorySetupPte (newPage);	// put new page as entry in pagetable
	}

	newPage = MemoryAllocPage ();	// allocating 1 page for the user stack
	if (newPage == 0) {				// if page was unable to be allocated
		printf ("bFATAL: couldn't allocate memory - no free pages!\n");
		exitsim ();	// NEVER RETURNS!
	}
	pcb->pagetable[L1_MAX_ENTRIES-1] = MemorySetupPte (newPage); // put new page as entry in pagetable

	newPage = MemoryAllocPage ();	// allocating 1 page for the system stack 
	if (newPage == 0) {				// if page was unable to be allocated
		printf ("cFATAL: couldn't allocate system stack - no free pages!\n");
		exitsim ();	// NEVER RETURNS!
	}
	pcb->sysStackArea = newPage * MEMORY_PAGE_SIZE; // setting the system stack area
...
//----------------------------------------------
  ...
  stackframe[PROCESS_STACK_PTSIZE] = L1_MAX_ENTRIES;	// setting the L1 size to the max number of entries allowed
...
{
	// add your code below
	QueueRemove(&pcb->l);	// remove process from list
	ProcessFreeResources (pcb);	// free the resources of the process
...
void PageFaultHandler()
{
  ...
  faultaddress = tempstackframe[PROCESS_STACK_FAULT];
	...
	if (currentPCB->npages == 13) { // if max number of pages has been reached, kill the process
		printf("Max memory reached --> Killing process...\n");
		return ProcessKill(currentPCB);
	}

	newPage = MemoryAllocPage();		// allocate new page
	if (newPage == 0) {					// if page was unable to be allocated
		printf ("dFATAL: couldn't allocate memory - no free pages!\n");
		return ProcessKill(currentPCB);	// kill the current process
	}

	page = faultaddress / MEMORY_PAGE_SIZE;					// set page 
	currentPCB->pagetable[page] = MemorySetupPte (newPage);	// put page as entry in table
	currentPCB->npages = currentPCB->npages + 1;  			// increment npages
}
