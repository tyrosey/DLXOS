...
//------------------------------------------
////////////////////////////////////////////////////////////////////////////////////////////////
	for (i = 0; i < L1_MAX_ENTRIES; i++) {			// for all entries to the L1 max
		if(pcb->pagetable[i] != 0) {				// if there is data in L1 pages
			for(j = 0; j < L2_MAX_ENTRIES; j++) {	// for all entries to the L2 max
				if( *((uint32 *) (pcb->pagetable[i] & MEMORY_PTE_MASK) + j)  != 0) {	// if there is data in L2 pages
					...
					MemoryFreePte (((uint32 *) (pcb->pagetable[i] * MEMORY_PTE_MASK)) + j ); // free the L2 memory
				}
				MemoryFreePte (pcb->pagetable[i]);	// free L1 memory
			}
		}
	}

  pcb->npages = 0;	// reset number of pages to 0
...
ProcessFork (VoidFunc func, uint32 param, char *name, int isUser)
{
  ...
  char *str;
  uint32 *L2_pagetable;	// declare pointer for L2 page tables
...
//------------------------------------------------------------
	...
	pcb->npages = 1;				
	newPage = MemoryAllocPage ();	// allocate the first L2 page table
	
	if (newPage == 0) {				// check if page was created successfully
		printf ("aFATAL: couldn't allocate memory - no free pages!\n");
		exitsim ();	// NEVER RETURNS!
	}
	
	pcb->pagetable[0] = MemorySetupPte (newPage);						// add page to page table for setup
	L2_pagetable = (uint32 *) (pcb->pagetable[0] & MEMORY_PTE_MASK);	// set address for L2 page table
  
	for(i = 0; i < 3; i++) {			// for loop allocating 3 pages for data and text for the new L2 page table						
		newPage = MemoryAllocPage ();	// allocate new page	
		if (newPage == 0) {				// check if page was created successfully
			printf ("bFATAL: couldn't allocate system stack - no free pages!\n");
			exitsim ();	// NEVER RETURNS!
		}
		*(L2_pagetable + i) = MemorySetupPte(newPage);	// setting the page address
	}
  
	newPage = MemoryAllocPage();	// allocating the last L2 page table
	
	if(newPage == 0) {				// check if page was created successfully 
		printf ("cFATAL: couldn't allocate system stack - no free pages!\n");
		exitsim ();	// NEVER RETURNS!
	}
	
	pcb->pagetable[L1_MAX_ENTRIES - 1] = MemorySetupPte(newPage);	// add page to page table for setup
	L2_pagetable = (uint32 *) (pcb->pagetable[L1_MAX_ENTRIES - 1] & MEMORY_PTE_MASK);	// setting the address for L2 page table
  
	newPage = MemoryAllocPage ();	// allocating 1 new page for user stack for L2
	if (newPage == 0) { 			// check if page was created successfully
		printf ("dFATAL: couldn't allocate system stack - no free pages!\n");
		exitsim ();	// NEVER RETURNS!
	}
	*(L2_pagetable + L2_MAX_ENTRIES - 1) = MemorySetupPte(newPage);	// setting the page address
  
	newPage = MemoryAllocPage ();	// allocating 1 new page for system stack for L2
	if (newPage == 0) {				// check if page was created successfully
		printf ("eFATAL: couldn't allocate system stack - no free pages!\n");
		exitsim ();	// NEVER RETURNS!
	}
...
//----------------------------------------------
  ...
  stackframe[PROCESS_STACK_PTSIZE] = L1_MAX_ENTRIES * L2_MAX_ENTRIES; // set the size of the overall page table
...
void ProcessKill (PCB *pcb)
{
	...
	QueueRemove(&pcb->l);		// remove process from queue
	ProcessFreeResources (pcb);	// free all resources of the process
...
void PageFaultHandler()
{
  ...
  int L2;
	...
	L1 = (fault_address / MEMORY_PAGE_SIZE) / L2_MAX_ENTRIES;	// 5 bits
	L2 = (fault_address / MEMORY_PAGE_SIZE) % L2_MAX_ENTRIES;	// 6 bits
  
  
	newPage = MemoryAllocPage();	// allocating memory page for L2 page table inside L1 entry
	
	if (newPage == 0) { 			// check if page was created successfully
		printf ("fFATAL: couldn't allocate system stack - no free pages!\n");
		return ProcessKill(currentPCB);
	}

	if(currentPCB->pagetable[L1] == 0) {						// if nothing is allocated to L1 page
		currentPCB->pagetable[L1] = MemorySetupPte (newPage);	// set new page to L1 entry
	}
	L2_pagetable = (uint32*)(currentPCB->pagetable[L1] & MEMORY_PTE_MASK);	// set new page's address

	newPage = MemoryAllocPage ();			// allocate memory page for page table
	
	if (newPage == 0) { 			// check if page was created successfully
		printf ("gFATAL: couldn't allocate system stack - no free pages!\n");
		return ProcessKill(currentPCB);
	}
	*(L2_pagetable = L2_pagetable + L2) = MemorySetupPte(newPage);	// setting up new page table entry

	currentPCB->npages = currentPCB->npages + 1;  					// increment nunmber of pages

	printf("Process id: %lu\tPage fault address: %p\t\tPhysical page number allocated: %d->%d\n",findpid(currentPCB),L2_pagetable+L2,L1,L2);
}