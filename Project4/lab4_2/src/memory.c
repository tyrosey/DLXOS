...
//-------------------------------------------
    ...
    int offset = addr % MEMORY_PAGE_SIZE;	// 13 bits
	int L1 = page / L2_MAX_ENTRIES; 		// 5 bits
	int L2 = page % L2_MAX_ENTRIES;			// 6 bits
	uint32 *L2_pagetable;					// L2 page table pointer
	
	if((L1*L2) > ((L1_MAX_ENTRIES-1)*(L2_MAX_ENTRIES-1))) {	// check to make sure enough memory is left
		return (0);
    }

    L2_pagetable = (uint32 *)(pcb->pagetable[L1] & MEMORY_PTE_MASK);	// initialize L2 pagetable pointer with L1 table
    L2_pagetable = L2_pagetable + L2;									// add previous table and the L2 tables

    return ((*(L2_pagetable) & MEMORY_PTE_MASK)+ offset);				// return physical address
}

//----------------------------------------------------------------------
//
