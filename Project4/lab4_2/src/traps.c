	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
	...
	printf ("Got a keyboard interrupt (char=0x%x(%c), nleft=%d)!\n",
      ...
      break;
    case TRAP_ACCESS:
      //printf ("cExiting after illegal access at iar=0x%x, isr=0x%x\n",
	      //iar, isr);
	  printf("Trap access\n");
      //exitsim ();
      ProcessKill(currentPCB);
    ...
    case TRAP_ADDRESS:
      ...
      printf ("bExiting after illegal address at iar=0x%x, isr=0x%x\n",
	      iar, isr);
      exitsim ();
      break;
    case TRAP_ILLEGALINST:
      pcb = currentPCB;
      printf("Current PROC %s\n", pcb->name);
      printf ("aExiting after illegal instruction at iar=0x%x, isr=0x%x\n",
	      iar, isr);
      exitsim ();
      break;
    case TRAP_PAGEFAULT:
