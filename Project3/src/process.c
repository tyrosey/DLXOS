...
char	debugstr[200];
static uint32 total_num_quanta = 0;	// variable for total quanta 
static uint32 time_start = 0;		// variable for the start of the timer

uint32 my_timer_get() {				// function for returning the time elapsed in seconds
...
ProcessModuleInit ()
{
  ...
  QueueInit (&freepcbs);
	...
	for (i = 0; i< NUM_OF_RUNQUEUE; i++) {			// for loop to initialize 32 run queues 
		QueueInit (&runQueue[i]);					// indexing the runQueue for 32 queues
	}
...
//  ProcessNextPrio
//
//  Search for the next runQueue with a process in it, then return
//  that process.
//
//----------------------------------------------------------------------

PCB *
ProcessNextPrio() 													// declare PCB * type function
{
	int i;															// declare int i for loop purposes
	PCB *returnVar = NULL;											// declare PCB * type variable and set to NULL

	for (i = 0; i < 32; i++) {										// for loop to shuffle through all 32 runQueues
		if ((&runQueue[i])->nitems != 0) {							// if there is something in a runQueue
			returnVar = (PCB *)(QueueFirst(&runQueue[i])->object);	// set return variable to the object located at said runQueue
			break;													// break from loop
		}
	}

	return returnVar;												// return the return variable
}
...
//  }
    ...
    currentPCB->quanta++;								// increment quanta
	total_num_quanta++;									// increment total number of quantas
	pcb = ProcessNextPrio();							// call function for finding the next process 
	currentPCB->runtime += my_timer_get() - time_start;	// finds runtime by subtracting the start time from the time returned from the timer function
	time_start = my_timer_get();						// set start time to the time returned by the timer function

	if (!pcb) {											// if there are no more proccesses to run
		printf ("No runnable processes - exiting!\n");
		exitsim ();										// exit
	}
...
//		printf(TIMESTRING3, currentPCB - pcbs, currentPCB->prio);
	...
	if (pcb == currentPCB) {									// if previous process is still highest priority
		currentPCB->estcpu++;									// increment estimated cpu time

		QueueRemove (&pcb->l);									// access the process for moving it
		QueueInsertLast (&runQueue[pcb->runQueueNum], &pcb->l);							// move to end of the queue
		
		if((currentPCB->quanta % 4) == 0) {												// 4 is the max number of quanta 
			currentPCB->prio = PUSER + (currentPCB->estcpu/4) + (2*currentPCB->p_nice);	// recalculate priority

			currentPCB->runQueueNum = currentPCB->prio/4;								// recalculating the run queue number
			QueueRemove(&currentPCB->l);												// access the process for moving it
			QueueInsertLast(&runQueue[currentPCB->runQueueNum], &currentPCB->l);		// move to end of queue
		}
	}

	if(total_num_quanta % 10 == 0) {		// 10 is max number of quanta
		for(i = 0; i < 32; i++) {			// for loop for each run queue 
			k = (&runQueue[i])->nitems;		// set k to number of items in the queue

			for (j = 0; j < k; j++) {								// for loop for as many items were in queue
				pcb = (PCB *)((QueueFirst(&runQueue[i]))->object);	// set pcb to the process at the front of the queue	
				pcb->estcpu = (int)((((float)2 * pcb->time_mod)/((float)2 * pcb->time_mod + 1)) * pcb->estcpu);  // decay the estimated CPU time of all processes
				pcb->prio = PUSER + (pcb->estcpu/4) + (2 * pcb->p_nice);     									// Recalculate the priority of all processes

				pcb->runQueueNum = pcb->prio/4;								// reset the queue number variable
				QueueRemove(&pcb->l);										// moving process from link
				QueueInsertLast(&runQueue[pcb->runQueueNum], &pcb->l);		// put process at end of queue
			}
		}

		pcb = ProcessNextPrio();													// call function for finding next process
		
		if (currentPCB == pcb) {													// if previous process is still highest priority
			QueueRemove(&currentPCB->l);											// moving process from link
			QueueInsertLast(&runQueue[currentPCB->runQueueNum], &currentPCB->l);	// move process to end of queue
		}
    }

    pcb = ProcessNextPrio();														// call function for finding next process

    currentPCB = pcb;																// set pcb to the current pcb
...
ProcessSuspend (PCB *suspend)
{
  ...
  suspend->sleeptime = my_timer_get();					// getting the time a process has been asleep
...
ProcessWakeup (PCB *wakeup)
{
	...
	sleep_time_sec = (my_timer_get() - wakeup->sleeptime)/1000;	// getting the sleep time in seconds

	if(sleep_time_sec >= 1) {			// if asleep longer than 1 second
		estcpu_temp = ((((float)2 * wakeup->time_mod)/((float)2 * wakeup->time_mod + 1)) * wakeup->estcpu); // getting the int for power in equation
		power = estcpu_temp;							// setting power to estimated cpu time
		for(i = 1; i < sleep_time_sec; i++) {			// for loop for as long as the process was asleep
			estcpu_temp = estcpu_temp * power;  		// To the power of sleep_time_sec
		}
		wakeup->estcpu = (int)estcpu_temp;				// waking up process
	}

	wakeup->prio = PUSER + (wakeup->estcpu/4) + (2*wakeup->p_nice);	// recalculating the priority after awakening
	wakeup->runQueueNum = wakeup->prio/4;	  						// resetting runqueue number
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  ...
  //--------------------------------------
	...
	if((isUser && p_nice < 0) || p_nice > 19) {  	// if p_nice is greater than 19
		pcb->p_nice = 0;							// reset p_nice to zero 
	}
	else {											// else leave it the hell alone
		pcb->p_nice = p_nice;	
	}
	
	// initialize all data in pcb struct 
	pcb->estcpu = 0;
	pcb->runtime = 0;
	pcb->sleeptime = 0;
	pcb->prio = PUSER;
  ...
  intrs = DisableIntrs ();
  QueueInsertLast (&runQueue[PUSER/4], l);	// placing the pcb in the run queue
  RestoreIntrs (intrs);
